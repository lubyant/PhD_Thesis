...
\citep{george_nearshore_2020}. Due to these characteristics, high-frequency
...
originate from two opposite directions (see Figure \ref{fig:c5_sites_climate}a). The
...
this study. Geomorphology is the one component of coastal
...
Lake Michigan, a periodic water level rise and fall were recorded over decades.
...
sediment transport and crosshore sediment transport  are parameters related to sedimentation.
 Table \ref{Summary of data sources} summarizes the data sources for these parameters.
...
\begin{equation}
    WHDI = \frac{|\sum Hs_{1}-\sum Hs_{2}|}{\sum Hs_{1}+\sum Hs_{2}}
\label{eq:eq5.3}
\end{equation}

where $Hs_{1}$ and $Hs_{1}$ represent wave heights weighted by significant wave
...
breaking wave height is estimated in the nearshore zone using Wave Routing Method
